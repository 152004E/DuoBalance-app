...
export function ExpenseParticipants({ participants }: ExpenseParticipantsProps) {
  return (
    <View
          ...
          <View key={p.name} className="flex-row items-center justify-between">
            <View className="flex-row items-center gap-3">
              <View className="flex h-10 w-10 items-center justify-center rounded-full bg-[#E2E8F0]">
                <Text className="text-sm font-bold text-[#64748B]">{p.initials}</Text>
              </View>
              <View>
                <Text className="text-base font-semibold text-[#0F172A]">{p.name}</Text>
                {p.isPayer && (
                  <View className="mt-0.5">
                    <Text className="text-[11px] font-semibold text-[#006c49]">Pagó</Text>
                  </View>
                )}
              </View>
            </View>
